...
// "chat" = just generated by the agent this session — offer to save it.
// "menu" = launched from a menu click (catalog or saved library) — it's
// either not savable (curated games) or already saved, so no prompt.
...
export function Workspace() {
  ...
  const handleLaunchSavedGame = useCallback((saved: SavedGame) => {
    setGame({ id: `library-${saved.id}`, spec: saved.spec, origin: "menu" });
  }, []);
...
function latestToolGame(messages: UIMessage[]): ActiveGame | null {
  for (let m = messages.length - 1; m >= 0; m--) {
    ...
    for (let p = parts.length - 1; p >= 0; p--) {
      ...
      if (parsed.success) {
        return { id: part.toolCallId, spec: parsed.data, origin: "chat" };
      }
    }
  }
  return null;
