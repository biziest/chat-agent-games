...
import type { GameSpec } from "@/lib/game";
import {
  chooseMove,
  EMPTY_BOARD,
  findWinner,
...
export function GamePane({ game }: { game: GameSpec | null }) {
  if (!game) return <EmptyState />;
  return <NoughtsAndCrosses game={game} />;
}
  ...
  return (
    <div className="flex flex-1 items-center justify-center p-10">
      <div className="max-w-sm text-center">
        ...
        <h2 className="text-base font-medium tracking-tight">No game yet</h2>
        <p className="mt-2 text-sm leading-relaxed text-muted">
          Ask for one in the chat — try{" "}
          <span className="text-foreground">
            &ldquo;create a simple game like tic tac toe&rdquo;
          </span>
          .
        </p>
      </div>
...
function NoughtsAndCrosses({ game }: { game: GameSpec }) {
