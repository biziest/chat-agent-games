...
| `app/components/workspace.tsx` | Owns every chat: a single `useChat()` call, but bound to a *different* AI SDK `Chat` object depending on what's on screen — one dedicated conversation per game (`WorkspaceState.sessions`), plus one always-on "lobby" chat for describing a brand-new game from the menu. A menu pick or saved-library launch gets a fresh chat seeded with a synthetic tool-call turn (`seedMessages`) so the model has the same context it'd have if the game had been chat-built, so "make it harder" works from any origin. Read `applyToolCall` before changing how a game gets created or edited — it's the one subtle part, especially the branch where the lobby chat's *own conversation* gets claimed as a new game's dedicated chat. |
