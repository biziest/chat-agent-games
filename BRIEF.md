...
| `lib/saved-games.ts` | Persistence for kept custom games — localStorage, not a database (no accounts in this app, so per-device is the right scope). Exposed as a `useSyncExternalStore`-shaped store (`subscribeSavedGames`/`getSavedGamesSnapshot`/`getSavedGamesServerSnapshot`/`writeSavedGames`), not a plain load function — reading localStorage in a `useEffect` + `setState` is exactly what `eslint-plugin-react-hooks`'s `set-state-in-effect` rule flags, and it's right to: that pattern also risks an SSR/hydration mismatch, which `useSyncExternalStore` is the actual sanctioned fix for. A `SavedGame` also carries an opaque `progress` field — see the progress note on `game-menu.tsx`/the board components below. |
...
| `app/components/workspace.tsx` | Owns every chat: a single `useChat()` call, but bound to a *different* AI SDK `Chat` object depending on what's on screen — one dedicated conversation per game (`WorkspaceState.sessions`), plus one always-on "lobby" chat for describing a brand-new game from the menu. A menu pick or saved-library launch gets a fresh chat with one real message already sent asking the model to reproduce the exact spec via a real tool call (`lib/resume-message.ts`) — **not** a client-side-only fake history splice, which was tried first and doesn't work: `chat.agent` reconstructs each session's history server-side from turns that actually happened, so a message that only exists in the browser's local `Chat` object is invisible to the model. Read `applyToolCall` before changing how a game gets created or edited — it's the one subtle part, especially the branch where the lobby chat's *own conversation* gets claimed as a new game's dedicated chat. |
| `lib/resume-message.ts` | The resume-turn protocol for menu/library-launched games — builds the `[[resume-game]] {...}` message text `workspace.tsx` sends, matched by the instructions in `trigger/chat.ts`'s system prompt (call the tool with that exact input) and by the display override in `chat-pane.tsx` (render it as a small "↻ Resumed X" badge instead of raw JSON). |
| `app/components/game-pane.tsx` | Dispatches on `spec.kind` to the right board component. Add a `case` here for each new game. |
| `app/components/game-menu.tsx`, `noughts-and-crosses-board.tsx`, `chess-board.tsx`, `custom-game-board.tsx` | The menu — the two curated games plus any saved custom games, grouped into genre folders (native `<details>`, not custom expand/collapse state) — and one board component per game. `custom-game-board.tsx` is a title, an optional "keep this game?" prompt, and a `sandbox="allow-scripts"` iframe — no rules engine, since the agent's HTML is the whole game. It also fetches the vendored `/vendor/three.min.js` once (module-level cached promise) and inlines it into the iframe's `srcDoc` ahead of the model's own `<script>`, so custom games get a `THREE` global without the model loading or bundling it itself — see `scripts/bundle-three.mjs` below for where that file comes from. The save prompt only shows for a game fresh out of chat (see `origin` on `ActiveGame` in `workspace.tsx`) — there's no way to detect "you finished playing" from inside the sandboxed iframe, so it's offered once at generation time rather than triggered by anything happening in-game. Deleting a saved game (`game-menu.tsx`'s `ConfirmDeleteModal`) requires typing "yes" first — it's the one destructive, unrecoverable action in the app. All three boards accept `initialProgress`/`onProgressChange`: each owns and validates its own resume-state shape (moves for the curated games; for custom games, an opaque blob the model reports via `postMessage` — see the save/restore contract in `trigger/chat.ts`'s system prompt) and reports changes up so `workspace.tsx` can keep it alive across a trip back to the menu (`GamePane` remounts the board every time — see its `key` comment — so nothing the board wants preserved can live in the board's own state). |
| `app/actions.ts` | Session creation + session-scoped token minting, plus the community-gallery actions (`publishGame`, `listPublishedGames`, `getPublishedGame`, `unpublishGame`) — the one place in this app that reads/writes state shared across every visitor, not just one browser. Called directly from client components (`custom-game-board.tsx`, `game-menu.tsx`, `published-game-board.tsx`) rather than threaded through props — normal for Next.js server actions. |
| `lib/published-games.ts`, `lib/db.ts`, `scripts/setup-db.mjs` | The shared gallery's types/validation, its Postgres query layer (`@vercel/postgres`), and the one-time setup script (`npm run setup:db`, needs `POSTGRES_URL` — see `.env.example`) — `CREATE TABLE IF NOT EXISTS` plus `ALTER TABLE ADD COLUMN IF NOT EXISTS` for each column added since, so it's safe to re-run against an already-live database. No accounts anywhere in this app, so publishing is anonymous — just a typed display name (`author_name`), not a login. |
| `lib/published-game-tracking.ts`, `app/components/type-to-confirm-modal.tsx` | Which published games *this browser* published (a localStorage list, not an enforced permission) is what gates the "Unpublish game" button in `custom-game-board.tsx` and `published-game-board.tsx` — there's no account system to check against. `TypeToConfirmModal` is the shared "type yes to confirm" dialog behind every irreversible action in the app (deleting a saved game, unpublishing a game). |
| `app/components/published-game-board.tsx` | Plays a game from the community gallery — read-only: no dedicated chat behind someone else's published game (the resume-turn contract assumes the model itself built the exact spec being resumed, which isn't true here), and no per-visitor progress persistence yet. Both are reasonable follow-ups, not fundamental limits. |
| `lib/three-runtime.ts` | `loadThreeSource`/`buildGameSrcDoc` factored out of `custom-game-board.tsx` so `published-game-board.tsx` can reuse the same three.js-injection logic without duplicating it. |
